...
const SEARCH_FIELDS = ["judul", "konten"];
...
function KnowledgeList() {
  ...
  const {
    ...
    totalFiltered,
    pageSize,
  } = usePaginatedSearch(entries, SEARCH_FIELDS);

  async function handleToggleAktif(entry) {
    setData((prev) =>
      prev.map((e) => (e.id === entry.id ? { ...e, aktif: !e.aktif } : e)),
    );
    try {
      await updateKnowledge(entry.id, { aktif: !entry.aktif });
    } catch (err) {
      setData((prev) =>
        prev.map((e) => (e.id === entry.id ? { ...e, aktif: entry.aktif } : e)),
      );
  ...
  return (
    <div>
      ...
      {entries && entries.length === 0 ? (
        <EmptyState label='Belum ada knowledge entry.' />
      ) : null}
      ...
      {entries && entries.length > 0 ? (
        <>
          ...
          {pageItems.length === 0 ? (
            <EmptyState
              label={`Tidak ada entry yang cocok dengan "${query}".`}
            />
          ) : (
              ...
              {pageItems.map((entry) => (
                ...
                >
                  <div className='min-w-0 flex-1'>
                    ...
                    <p className='mt-1 line-clamp-2 whitespace-pre-line text-sm text-black/50'>
                      {entry.konten}
                    </p>
                  </div>
                  <div className='flex shrink-0 items-center gap-3'>
                    <Toggle
                      checked={entry.aktif}
                      onChange={() => handleToggleAktif(entry)}
                    />
                    <Link
