...
const SEARCH_FIELDS = ["nama", "kategori", "jawaban"];
...
function IntentList() {
  ...
  const { query, setQuery, page, setPage, totalPages, pageItems, totalFiltered, pageSize } =
    usePaginatedSearch(intents, SEARCH_FIELDS);
        ...
        <>
          <div className="mb-3">
            <SearchInput
              value={query}
              onChange={setQuery}
              placeholder="Cari nama intent, kategori, atau jawaban..."
            />
          </div>

          {pageItems.length === 0 ? (
            <EmptyState label={`Tidak ada intent yang cocok dengan "${query}".`} />
          ) : (
